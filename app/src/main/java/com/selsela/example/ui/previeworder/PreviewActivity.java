...
import static com.selsela.example.ui.payment.PaymentActivity.CASH;
import static com.selsela.example.ui.payment.PaymentActivity.KNET;
...
public class PreviewActivity extends BaseActivity implements PaymentMvpView {
    ...
    private void fillWithData() {
        ...
        service = preferencesHelper.getCurrentUserSetting().getConfig().getService_cost();
        ...
        if (orderBody.getCoupon() != null) {
            ...
            String discountValue = AppUtils.calculateEquation(orderBody.getPrice() + "-" + ratioPrice);
            discountvalueTextView.setText(discountValue + " " + getCurrency());
            price = AppUtils.calculateEquation(orderBody.getPrice() + "-" + discountValue);
        }
        transportCostValue.setText(transportCost + " " + getCurrency());
        price = AppUtils.calculateEquation(price + " + " + transportCost + "+" + service);
            ...
            if (productOrderList != null && productOrderList.size() > 0) {
                ...
                if (!country.getPrefix().equals(Const.KUWAIT)) {
                    if (boxes != null && boxes.size() > 0) {
                        ...
                        transportCost = getMinBox(totalWeight) * preferencesHelper.getCountry().getConversionFactor();
