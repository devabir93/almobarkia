...
public class BaseActivity extends AppCompatActivity implements MvpView, InternetConnectivityListener {
    ...
    public List<Box> getShippingBoxes() {
        return preferencesHelper.getShippingBoxes() != null ? preferencesHelper.getShippingBoxes().getBoxs() : null;
    }
    ...
    protected void reOpenApp() {
        this.finish();
        Intent intent = new Intent(this, SplachScreenActivity.class);
        intent.putExtra("finish", true); // if you are checking for this in your other Activities
        intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP |
                Intent.FLAG_ACTIVITY_CLEAR_TASK |
                Intent.FLAG_ACTIVITY_NEW_TASK);
